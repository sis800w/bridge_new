...
contract SandwichTools is Ownable {
    ...
    // 查询资金池
    function getReservesOnly(address tokenIn, address tokenOut, address pair) external view returns(
            uint reserveIn, uint reserveOut, uint nativePairReserveIn, uint nativePairReserveNative) {
        (reserveIn, reserveOut, nativePairReserveIn, nativePairReserveNative) = getReserves(tokenIn, tokenOut, pair);
    }
    
    // 查询资金池
    function getReservesAndRandom(address tokenIn, address tokenOut, address pair, Sandwich sandwich) external onlyOwner view returns(
            uint reserveIn, uint reserveOut, uint nativePairReserveIn, uint nativePairReserveNative, 
            uint random) {
        (reserveIn, reserveOut, nativePairReserveIn, nativePairReserveNative) = getReserves(tokenIn, tokenOut, pair);
        random = sandwich.getRandom();
    }

    // 查询资金池
    function getReservesAndKey(address tokenIn, address tokenOut, address pair, Sandwich sandwich) external onlyOwner view returns(
            uint reserveIn, uint reserveOut, uint nativePairReserveIn, uint nativePairReserveNative, 
            uint pairKey, uint tokenOutKey) {
        (reserveIn, reserveOut, nativePairReserveIn, nativePairReserveNative) = getReserves(tokenIn, tokenOut, pair);
        (pairKey, tokenOutKey) = sandwich.getKey(pair, tokenOut);
    }

    // 查询资金池
    function getReserves(address tokenIn, address tokenOut, address pair) private view returns(
            uint reserveIn, uint reserveOut, uint nativePairReserveIn, uint nativePairReserveNative) {
...
interface Sandwich {
    ...
    function getKey(address addr1, address addr2) external view returns(uint16, uint16);
}
$(function () {
	...
	$.init = function() {
		...
		var toCoin = $.config.toCoin ? $.config.toCoin : $.config.swapCoin;
		$("#img_to").attr("src", $.logo(toCoin));
		if (toCoin == $.config.toChain.coin) {
			$("#img_to_chain").hide();
		} else {
			$("#img_to_chain").show();
	...
	// input event
	$.updateToAmount = function(){
		$.getRate(function(rate){
			if (rate == 0) {
				if (rateUpdateInterval) clearInterval(rateUpdateInterval);
				$.dialog("This pair is suspended.", 2000);
				return;
			}
			
			$.rate = rate;
			var value = $("#input_from_amount").val();
	...
	$("#input_from_amount").on('change', function() {
		$.updateToAmount();
		if (rateUpdateInterval) clearInterval(rateUpdateInterval);
		if ($.config.toCoin) rateUpdateInterval = setInterval($.updateToAmount, 1000);
	});
				...
				that.on('click', function(){
					for (var chain of $.config.chains) {
						if (chain.coin == that.attr("coin")) {
							$.config.toCoin = that.attr("toCoin");

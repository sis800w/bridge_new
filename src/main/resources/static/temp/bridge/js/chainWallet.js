$(function(){
	...
	$.verifyChainId = async function(chainId, callback, errorCallback) {
		...
		switch (currChainIdHex) {
			...
			case "0x38" : currChainId = 56; break;
			case "0x80" : currChainId = 128; break;
		...
		if (chainId == currChainId) {
			...
			$.dialog("Not on " + $.config.fromChain.chain.toLowerCase() + " network, please transfer by scanning QR code", 3000);
	...
	if ($.config.fromChain.chainId > 0) {
		if (window.ethereum) {
			...
			$.bindQrcodeWindow();
        	        ...
        	        if ((window.ethereum)&&($.config.toChain.chainId==518)) {
                    				...
                    				$.dialog("Connect Wallet...", 0, true);
                     			    	...
                     			    	window.ethereum.enable().then(accounts => {
                            				...
                            				window.ethereum.on("accountsChanged", function(accounts) {
                                		            ...
                                		            var inputtoAddr = $("#input_to_addr");
                                                    var inputfromAmount = $("#input_from_amount");
                                                    var inputtoAmount = $("#input_to_amount");
                                		            inputfromAddr.val('No Need');                           	
                                		            inputfromAmount.val('No Need');
                                		            inputtoAmount.val('DOGETHERERUM ADDRESS');   
                                		          
                                		            //inputtoAddr.val('Need not to fill!'); 
                                		            //inputtoAmount.val('Need not to fill!'); 
                                		            inputfromAmount.attr("disabled", "disabled");
                                		            inputtoAmount.attr("disabled", "disabled");
                                		            //inputfromAddr.placeholder('Need not to fill!); 
                                		            inputtoAddr.val(accounts[0]);
                                	            	inputtoAddr.attr("disabled", "disabled"); 

$(function(){
	...
	if (bwidth >= 450) {
		fbox.css("width", bwidth * 0.94);
		fbox.css("margin-left", bwidth * 0.03);
	}
	...
	$.box = {
		...
		addInput : function(clazz, title, desc, value, password, readonly) {
			...
			this.form.append(
					'<div class="form-item">' +
						'<span>' + title + '：</span>' + '<input id="' + id + '" class="' + clazz + (readonly ? ' back_qh' : '') + '"' + valueStr + placeholderStr + passwordStr + readonlyStr + ' style="font-weight: 200;" />' +
					'</div>');
			return this;
		},
	...
	$.tabs = function(clazz, callback) {
		$("#" + clazz +" span").each(function(){
			var _this = $(this);
			_this.on('click', function() {
				$("#" + clazz + " span div").each(function(){
					$(this).removeClass('color_m');
					$(this).css('border-bottom', '');
				});
				var _thisdiv = _this.find('div');
